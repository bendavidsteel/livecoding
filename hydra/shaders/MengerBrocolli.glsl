/**
 * @file MengerBrocolli.glsl
 *
 * @brief This shader targets to achieve a mathematical render of Menger's Brocolli, a fractal based on the more
...
vec4 RayMarcher (vec3 ro, vec3 rd) {
  ...
  for (steps = 0.0; steps < float (MaximumRaySteps); steps++) {
    ...
    totalDistance += distance; // Increases the total distance marched
